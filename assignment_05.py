...
# -*- coding: utf-8 -*-
"""
Created on Thu Sep 29 19:33:31 2022
...
def exercise01():
    ...
    df = pd.DataFrame([[11,34,2,94],[5,16,23,2],[1,97,45,66]])    
...
def exercise08():
    ...
    row_count = df.shape[0] #returns the number of rows
    avg_sq_ft = np.mean(df['sq__ft']) #calculate mean of sq__ft column
    df_zip_95670 = df[df['zip'] == 95670] # subsetting using conditional statement when zip is 95670
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = np.flip(np.indices((n*2,n*2)).sum(axis = 0) % 2, axis = 1)
...
def exercise15():
    ...
    data = pd.read_csv('https://tinyurl.com/y63q7okz')
    df = data.iloc[list(range(4,len(data),5)),[0,2]]
